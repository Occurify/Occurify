...
public class TimelineSkipLastWithinTests
{
    ...
    private void ExecuteTest(TimelineMethods method, string source, string periods, int skipCount, string expected)
    {
        ...
        // Act
        var result = timeline.SkipLastWithin(periodTimeline, skipCount);
    ...
    {
        using var r = new StreamReader("TestCases/Timeline.SkipLastWithin.json");
        ...
        return testCases.SelectMany(cases =>
            cases.Select(tc => new object[]
            {
                tc.Source ?? throw new InvalidOperationException(
                    $"{nameof(tc.Source)} of null is not supported in {nameof(TimelineSkipLastWithinTests)}."),
                tc.Periods ?? throw new InvalidOperationException(
                    $"{nameof(tc.Periods)} of null is not supported in {nameof(TimelineSkipLastWithinTests)}."),
                tc.Skip ?? throw new InvalidOperationException(
                    $"{nameof(tc.Skip)} of null is not supported in {nameof(TimelineSkipLastWithinTests)}."),
                tc.Expected ?? throw new InvalidOperationException(
                    $"{nameof(tc.Expected)} of null is not supported in {nameof(TimelineSkipLastWithinTests)}.")
            })).ToArray();
    }
}