...
[TestClass]
public class TimelineTakeWithinTests
{
    [DataTestMethod]
    ...
    private void ExecuteTest(TimelineMethods method, string source, string periods, int takeCount, string expected)
    {
        ...
        // Act
        var result = timeline.TakeWithin(periodTimeline, takeCount);
    ...
    {
        using var r = new StreamReader("TestCases/Timeline.TakeWithin.json");
        ...
        return testCases.SelectMany(cases =>
            cases.Select(tc => new object[]
            {
                tc.Source ?? throw new InvalidOperationException(
                    $"{nameof(tc.Source)} of null is not supported in {nameof(TimelineTakeWithinTests)}."),
                tc.Periods ?? throw new InvalidOperationException(
                    $"{nameof(tc.Periods)} of null is not supported in {nameof(TimelineTakeWithinTests)}."),
                tc.Take ?? throw new InvalidOperationException(
                    $"{nameof(tc.Take)} of null is not supported in {nameof(TimelineTakeWithinTests)}."),
                tc.Expected ?? throw new InvalidOperationException(
                    $"{nameof(tc.Expected)} of null is not supported in {nameof(TimelineTakeWithinTests)}.")
            })).ToArray();
    }
}