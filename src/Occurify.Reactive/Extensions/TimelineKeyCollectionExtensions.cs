...
/// </summary>
public static class TimelineKeyCollectionExtensions
{
    /// <summary>
        ...
        if (emitSampleUponSubscribe)
        {
            ...
            return Observable.Defer(() =>
                source.ToSampleObservableInternal(utcNow, scheduler)
                    .Prepend(new KeyValuePair<DateTime, TValue[]>(utcNow, source.GetKeysAtUtcInstant(utcNow))));
        }
        ...
        if (emitSampleUponSubscribe)
        {
            ...
            return Observable.Defer(() =>
                source.ToSampleObservableInternal(relativeTo, scheduler)
                    .Prepend(new KeyValuePair<DateTime, TValue[]>(relativeTo, source.GetKeysAtUtcInstant(relativeTo))));
        }
