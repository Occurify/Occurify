...
public static partial class PeriodTimelineExtensions
{
    ...
    /// If <paramref name="includeCurrentState"/> is true, the state at the current time will be emitted immediately upon subscribing.
    /// </summary>
    public static IObservable<bool> ToBooleanObservable(this IPeriodTimeline periodTimeline, IScheduler scheduler, bool includeCurrentState = true) =>
        periodTimeline.ToSampleObservable(scheduler, includeCurrentState).Select(s => s.IsPeriod);
    ...
    /// If <paramref name="includeCurrentState"/> is true, the state at <paramref name="relativeTo"/> will be emitted immediately upon subscribing.
    /// </summary>
    public static IObservable<bool> ToBooleanObservable(this IPeriodTimeline periodTimeline, DateTime relativeTo, IScheduler scheduler, bool includeCurrentState = true) => 
        periodTimeline.ToSampleObservable(relativeTo, scheduler, includeCurrentState).Select(s => s.IsPeriod);

    /// <summary>
    /// Returns a <c>IObservable</c> that emits a <see cref="PeriodTimelineSample"/> every time a period starts or ends.
    /// If <paramref name="includeCurrentSample"/> is true, the sample at the current time will be emitted immediately upon subscribing.
    /// </summary>
    public static IObservable<PeriodTimelineSample> ToSampleObservable(this IPeriodTimeline periodTimeline,
        IScheduler scheduler, bool includeCurrentSample = true) => periodTimeline.ToSampleObservable(DateTime.UtcNow, scheduler, includeCurrentSample);
    ...
    /// If <paramref name="includeCurrentSample"/> is true, the sample at <paramref name="relativeTo"/> will be emitted immediately upon subscribing.
    /// </summary>
    public static IObservable<PeriodTimelineSample> ToSampleObservable(this IPeriodTimeline periodTimeline,
        DateTime relativeTo, IScheduler scheduler, bool includeCurrentSample = true)
    {
