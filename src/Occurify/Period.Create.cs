...
public partial record Period
{
    ...
    /// Both instants have to be UTC.
    /// </summary>
    public static Period Create(DateTime? start, DateTime? end)
    {
        if (start != null && start.Value.Kind != DateTimeKind.Utc)
            throw new ArgumentException("Start should be UTC time.", nameof(start));
        if (end != null && end.Value.Kind != DateTimeKind.Utc)
            throw new ArgumentException("End should be UTC time.", nameof(end));
        if (end < start)
    ...
    /// <paramref name="start"/> has to be UTC.
    /// </summary>
