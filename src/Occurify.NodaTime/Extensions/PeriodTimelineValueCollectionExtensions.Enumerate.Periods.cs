...
public static partial class PeriodTimelineValueCollectionExtensions
{
    ...
    /// <summary>
    /// Enumerates all periods on <paramref name="source"/> within <paramref name="interval"/> from earliest to latest and returns the period along with the values of the timelines that include this exact period.
    /// <paramref name="periodIncludeOptions"/> defines inclusion of periods around the start and end of <paramref name="interval"/>.
    /// Periods are ordered using <see cref="Period.CompareTo"/>. Duplicates are removed.
    /// </summary>
    public static IEnumerable<KeyValuePair<Period, TValue[]>> EnumeratePeriod<TValue>(this IEnumerable<KeyValuePair<IPeriodTimeline, TValue>> source, Interval interval, PeriodIncludeOptions periodIncludeOptions = PeriodIncludeOptions.CompleteOnly) =>
        source.EnumeratePeriod(interval.ToPeriod(), periodIncludeOptions);

    /// <summary>
    /// Enumerates all periods on <paramref name="source"/> within <paramref name="interval"/> from latest to earliest and returns the period along with the values of the timelines that include this exact period.
    /// <paramref name="periodIncludeOptions"/> defines inclusion of periods around the start and end of <paramref name="interval"/>.
    /// Periods are ordered using <see cref="Period.CompareTo"/>. Duplicates are removed.
    /// </summary>
    public static IEnumerable<KeyValuePair<Period, TValue[]>> EnumeratePeriodBackwards<TValue>(this IEnumerable<KeyValuePair<IPeriodTimeline, TValue>> source, Interval interval, PeriodIncludeOptions periodIncludeOptions = PeriodIncludeOptions.CompleteOnly) =>
