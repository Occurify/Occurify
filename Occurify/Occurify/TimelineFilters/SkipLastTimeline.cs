...
internal class SkipLastTimeline : Timeline
{
    private readonly ITimeline _source;
    private readonly IPeriodTimeline _mask;
    private readonly int _count;

    public SkipLastTimeline(ITimeline source, IPeriodTimeline mask, int count)
    {
        if (count < 0)
        {
    ...
    public override DateTime? GetPreviousUtcInstant(DateTime utcRelativeTo)
    {
        ...
        do
        {
            ...
            {
                if (TryFindPreviousInstantInPeriod(period, previous.Value, out DateTime? actual))
                {
                    return actual;
                }
    ...
    public override DateTime? GetNextUtcInstant(DateTime utcRelativeTo)
    {
        ...
        do
        {
            ...
            {
                if (EnumerateBackwardsFrom(period.End).Take(_count).All(i => i != next))
                {
                    return next;
                }
    ...
    private bool TryFindPreviousInstantInPeriod(Period period, DateTime proposed, out DateTime? actual)
    {
        ...
        var instantBeforeSkip = _source.GetPreviousUtcInstant(lastInstantToSkip.Value);
        if (instantBeforeSkip == null)
        {
            actual = null;
            return true;
        }
        if (period.ContainsInstant(instantBeforeSkip.Value))
        {
            actual = instantBeforeSkip;
            return true;
        }
    ...
    public override bool IsInstant(DateTime utcDateTime)
    {
        ...
        return EnumerateBackwardsFrom(period.End).Take(_count).All(instant => instant != utcDateTime);
    }
