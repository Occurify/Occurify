...
[TestClass]
public class PeriodTimelineWithoutTests
{
    [DataTestMethod]
    ...
    private void ExecuteTest(TimelineMethods method, string source, string periods, string expected)
    {
        ...
        // Act
        var filteredPeriodTimeline = periodTimeline.Without(maskPeriodTimeline);
    ...
    {
        using var r = new StreamReader("TestCases/PeriodTimeline.Without.json");
        var json = r.ReadToEnd();
        var testCases = JsonConvert.DeserializeObject<PeriodTimelineWithoutTestCase[][]>(json) ?? throw new InvalidOperationException("Was unable to load test cases.");
        return testCases.SelectMany(cases =>
            cases.Select(tc => new object[]
            {
                tc.Source ?? throw new InvalidOperationException(
                    $"{nameof(tc.Source)} of null is not supported in {nameof(PeriodTimelineWithoutTests)}."),
                tc.Periods ?? throw new InvalidOperationException(
                    $"{nameof(tc.Periods)} of null is not supported in {nameof(PeriodTimelineWithoutTests)}."),
                tc.Expected ?? throw new InvalidOperationException(
                    $"{nameof(tc.Expected)} of null is not supported in {nameof(PeriodTimelineWithoutTests)}.")
            })).ToArray();
    }
}